...
options3 = [];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Bootstrap = 0; % If you want to bootstrap to generate confidence intervals 
               % just set Bootstrap = 1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Experimental Data
...
Proportion = [22.8,6.7,18,13.7,12.8,11.7,8,6.4]/100; % Queens-NY, USA
PropInfections = [1181.64,3542.22,3963.07,4576.72,5179.13,5364.26,4799.84,5396.81];
PropInfections = PropInfections/max(PropInfections);
factor = [0.852264603,1.580510353,0.838658147,0.951782546,0.923559919,1.116377425,1.28792743,1.464768993];
PropInfections = PropInfections.*factor;
PropHosp = [0.0416,0.0569,0.0728,0.1039,0.1531,0.2406,0.4013,0.5768];
factor = [5.024390244,4.537216828,5.214549938,5.323353293,5.161311054,5.155095412,5.345340642,5.443145925];
PropHosp = factor.*PropHosp;
PropICU = ones(size(PropHosp));%min(1,[0.01926,0.07831,0.22251,0.3786,0.5973]/0.4);
PropDeath = [0.019,0.0246,0.0446,0.0969,0.1609,0.2401,0.3522,0.5573];
factor = [3.2,3.181818182,3.114754098,3.107142857,3.161835749,3.096385542,3.061829653,3.055468011];
...
Death(3:end) = min(2,data2(2:end,4)./data2(1:end-1,3));
